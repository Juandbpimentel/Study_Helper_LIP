...
=        <div className="flex-1 overflow-y-auto p-8 bg-white custom-scrollbar">
          <div className="space-y-10">
            {weekDays.map((dayName, dayIndex) => (
              <div key={dayIndex} className="animate-in slide-in-from-bottom-2 duration-300" style={{ animationDelay: `${dayIndex * 50}ms` }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {subjects.map((subject) => {
                    const isSelected = scheduleMap[dayIndex]?.includes(subject.id);
                    return (
                      <button
                        key={subject.id}
                        onClick={() => toggleSubject(dayIndex, subject.id)}
                        className={`
                          group relative flex items-center gap-3 p-4 rounded-xl border transition-all duration-200 text-left w-full
                          ${isSelected 
                        ...
                        `}
                      >
                        <div className={`shrink-0 transition-colors duration-200 ${isSelected ? "text-indigo-600" : "text-slate-300 group-hover:text-indigo-300"}`}>
                          {isSelected ? (
                            <CheckSquare className="w-6 h-6 fill-indigo-50" strokeWidth={2} />
                          ) : (
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          />
                          <span className={`text-sm font-medium truncate ${isSelected ? "text-slate-900" : "text-slate-600"}`}>
                            {subject.tema}
                          </span>
                        </div>
                      </button>
        ...
        <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-white z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          <button
            onClick={onClose}
            className="px-6 py-2.5 rounded-xl font-medium text-slate-600 hover:bg-slate-100 border border-slate-200 transition-all active:scale-[0.98]"
            disabled={isSaving}
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="px-6 py-2.5 rounded-xl font-medium text-white bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all flex items-center gap-2 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isSaving ? (
              <>
